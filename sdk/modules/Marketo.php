...
class Marketo {
	...
	public static function export($client, array $get = array(), array $post = array()) {
		$attributes = array("GET" => $get, "POST" => $post);
		$json = $client->getJSONObject("marketo", "export", $attributes);
		return $json;
	}
	public static function syncAvailableFields($client, array $get = array(), array $post = array()) {
		$attributes = array("GET" => $get, "POST" => $post);
		$json = $client->getJSONObject("marketo", "syncAvailableFields", $attributes);
		return $json;
	}
}
?>